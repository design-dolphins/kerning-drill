...
const randomInitialKerning = () => Math.round((Math.random() * 100) / 5) * 5;
...
const teachingTargets: Record<string, number[]> = {
  ...
  "日本語:Zen Kaku Gothic New:ゆめみる":[5,-5,-45], "日本語:Zen Kaku Gothic New:きらり":[0,-40], "日本語:Zen Kaku Gothic New:おとずれ":[-60,-55,5], "日本語:Zen Kaku Gothic New:やさしい":[0,-15,-10], "日本語:Zen Kaku Gothic New:静かな休日":[0,0,0,-25], "日本語:Zen Kaku Gothic New:今日はいい日":[5,-15,0,0,-20], "日本語:Zen Kaku Gothic New:夜の図書館":[5,-5,-20,0], "日本語:Zen Kaku Gothic New:あさひ":[-20,0], "日本語:Zen Kaku Gothic New:みなと":[5,-55], "日本語:Zen Kaku Gothic New:かがやき":[5,5,-10], "日本語:Zen Kaku Gothic New:静かな場所":[-5,-5,-5,10], "日本語:Zen Kaku Gothic New:街角の書店":[-5,-55,-5,-5], "日本語:Zen Kaku Gothic New:日々の記録":[-90,-65,5,10], "日本語:Zen Kaku Gothic New:たまご":[5,-35], "日本語:Zen Kaku Gothic New:なつやすみ":[-25,-10,0,-50], "日本語:Zen Kaku Gothic New:てのひら":[-20,5,-30]
};
...
export default function KerningDrill() {
  ...
  const [comparisonView, setComparisonView] = useState<"blend" | "user" | "target">("blend");
  ...
  const init = (next: Drill) => {
    setDrill(next);
    const startingValues = Array.from({ length: next.text.length - 1 }, (_, index) => isFixedSpace(index, next) ? pairAt(next, index).target : randomInitialKerning());
    setInitialValues(startingValues);
  ...
  const targets = useMemo(() => customTargets[customTargetKey]?.length === drill.text.length - 1 ? customTargets[customTargetKey] : teachingTargets[customTargetKey]?.length === drill.text.length - 1 ? teachingTargets[customTargetKey] : metricTargets.length === drill.text.length - 1 ? metricTargets : Array(drill.text.length - 1).fill(0), [drill, metricTargets, customTargets, customTargetKey]);
  ...
  const moveSelection = (direction: number) => { setCaretVisible(true); setSelected(current => { let next=current + direction; while(next >= 0 && next < values.length && isFixedSpace(next)) next += direction; return Math.max(-1, Math.min(values.length, next)); }); };
  useEffect(() => { const handler=(e:KeyboardEvent)=>{ if(screen!=="drill" || !["ArrowLeft","ArrowRight"].includes(e.key)) return; e.preventDefault(); if(e.altKey) { update(selected, values[selected]+(e.key==="ArrowLeft"?-10:10)); return; } moveSelection(e.key==="ArrowLeft"?-1:1); }; window.addEventListener("keydown",handler); return()=>window.removeEventListener("keydown",handler); });
  useEffect(() => {
  ...
  const goHome = () => { setScreen("home"); setShowMetrics(false); setCaretVisible(false); };
  const next = () => {
  ...
  const selectedAdjustment = selected >= 0 && selected < values.length ? values[selected] - (initialValues[selected] ?? values[selected]) : 0;
  const userComparisonOpacity = comparison <= 50 ? 1 : 1 - 0.9 * ((comparison - 50) / 50);
  const targetComparisonOpacity = comparison <= 50 ? 0.1 + 0.4 * (comparison / 50) : comparison / 100;
  ...
  return <main className="min-h-screen px-5 py-5 sm:px-10 sm:py-8">
    ...
    {screen !== "home" && <section className="mx-auto max-w-6xl py-9"><div className="flex items-center justify-between"><div><p className="text-xs font-medium tracking-[.12em] text-[#6e6e73]">{isReview ? "復習" : drill.level} · DRILL　{guidedIndex + 1}/{guidedQueue.length || 12}</p><h2 className="mt-1 text-xl font-medium tracking-[-.03em]">余白を見ながら、文字を選択</h2></div><p className="text-xs text-[#6e6e73]">{editorMode && <span className="mr-3 text-[#146ef5]">編集モード</span>}フォント　<span className="text-[#171719]">{font}</span></p></div>
      <div className={`mt-8 border-y hairline ${screen === "result" ? "min-h-[300px] py-10" : "min-h-[270px] py-10 sm:min-h-[390px] sm:py-16"}`}><div className="overflow-x-auto px-2 text-center"><div className="relative inline-block text-left"><KerningText text={drill.text} values={renderedValues} font={font} selected={selected} onSelect={activatePair} drag={drag} update={update} isFixedSpace={isFixedSpace} caretVisible={screen === "drill" && caretVisible} opacity={screen === "result" ? comparisonView === "user" ? 1 : comparisonView === "target" ? 0 : userComparisonOpacity : 1} />{(screen==="result" || showMetrics) && <div className="pointer-events-none absolute left-0 top-0" style={{opacity: screen === "result" ? comparisonView === "user" ? 0 : comparisonView === "target" ? 1 : targetComparisonOpacity : 0.85}}><KerningText text={drill.text} values={targets} font={font} selected={-1} onSelect={()=>{}} drag={drag} update={()=>{}} isFixedSpace={isFixedSpace} caretVisible={false} blue /></div>}</div></div></div>
      {screen === "drill" && editorMode && <div className="mt-5 flex flex-wrap items-center gap-3"><button onClick={()=>setShowMetrics(!showMetrics)} className="border px-4 py-2 text-sm font-medium hairline">{showMetrics ? "メトリクスを隠す" : "メトリクスを見る"}</button>{showMetrics && selected >= 0 && selected < targets.length && <p className="text-xs text-[#146ef5]">メトリクス　{targets[selected] > 0 ? "+" : ""}{targets[selected]} /1000 em</p>}</div>}
      {screen === "drill" ? <div className="mt-7 flex flex-col justify-between gap-7 sm:flex-row sm:items-end"><div>{selected < 0 || selected >= values.length ? <div className="text-sm text-[#6e6e73]">{selected < 0 ? "単語の先頭" : "単語の末尾"}　<span className="text-xs">この位置ではカーニングできません</span></div> : <><div className="flex items-center gap-3 text-sm"><span className="font-medium">{`${drill.text[selected]}${drill.text[selected+1]}`}</span><span className="font-mono text-[#6e6e73]">{touchedPairs[selected] ? <>{selectedAdjustment > 0 ? "+" : ""}{selectedAdjustment}</> : "—"}</span><span className="text-xs text-[#6e6e73]">/1000 em</span></div><div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4"><div className="flex w-full items-center gap-2 sm:w-auto"><button onClick={()=>moveSelection(-1)} aria-label="前の文字間を選択" className="grid h-9 w-8 shrink-0 place-items-center text-xl text-[#6e6e73]">‹</button><button onClick={()=>update(selected,values[selected]-1)} aria-label="1詰める" className="grid h-9 w-9 shrink-0 place-items-center border text-lg hairline">−</button><input className="track min-w-0 flex-1 sm:w-80 sm:flex-none" type="range" min="-180" max="180" value={touchedPairs[selected] ? selectedAdjustment : 0} onChange={e=>update(selected,(initialValues[selected] ?? 0)+ +e.target.value)} /><button onClick={()=>update(selected,values[selected]+1)} aria-label="1広げる" className="grid h-9 w-9 shrink-0 place-items-center border text-lg hairline">＋</button><button onClick={()=>moveSelection(1)} aria-label="次の文字間を選択" className="grid h-9 w-8 shrink-0 place-items-center text-xl text-[#6e6e73]">›</button></div><span className="hidden text-xs text-[#6e6e73] sm:inline">または <span className="key">⌥</span> + <span className="key">←</span><span className="key">→</span><span className="ml-2">単語間スペースは固定</span></span><span className="text-xs leading-5 text-[#6e6e73] sm:hidden">文字間をタップして、スライダーまたは左右ドラッグで調整</span></div></>}</div><button onClick={submit} className="bg-[#171719] px-5 py-3 text-sm font-medium text-white">採点する <span className="ml-5 text-[#b4b4b8]">→</span></button></div> : <Result drill={drill} values={values} targets={targets} accuracy={accuracy} comparison={comparison} setComparison={setComparison} comparisonView={comparisonView} setComparisonView={setComparisonView} next={next} saveCurrentAnswerAsTarget={saveCurrentAnswerAsTarget} restoreTeachingTarget={restoreTeachingTarget} hasCustomTarget={Boolean(customTargets[customTargetKey])} />}</section>}
  </main>;
}

function KerningText({text,values,font,selected,onSelect,drag,update,isFixedSpace,caretVisible,blue=false,opacity=1}:{text:string;values:number[];font:string;selected:number;onSelect:(n:number)=>void;drag:React.MutableRefObject<{x:number;value:number}|null>;update:(n:number,v:number)=>void;isFixedSpace:(index:number)=>boolean;caretVisible:boolean;blue?:boolean;opacity?:number}) { const isJapanese = /[ぁ-んァ-ヶ一-龠々]/.test(text); return <div aria-label="Kerning canvas" style={{fontFamily:font,color:blue?"#146ef5":"#171719",opacity}} className={`inline-block whitespace-nowrap text-[clamp(56px,11vw,130px)] font-medium leading-none ${isJapanese ? "tracking-normal" : "tracking-[-.055em]"}`}>{[...text].map((ch,i)=>{const fixed=i>=text.length-1 || isFixedSpace(i);const atStart=selected===-1&&i===0;const atEnd=selected===values.length&&i===text.length-1;return <span key={i} className={`char ${selected===i&&!blue&&!fixed?"active":""}`} style={{marginRight:i<text.length-1?`${values[i]/1000}em`:0, transition:"margin-right .4s cubic-bezier(.2,.8,.2,1)"}} onClick={()=>!blue&&!fixed&&onSelect(i)} onPointerDown={e=>{if(blue||fixed)return; onSelect(i); drag.current={x:e.clientX,value:values[i]}; e.currentTarget.setPointerCapture(e.pointerId)}} onPointerMove={e=>{if(!drag.current||blue||fixed)return; update(i,drag.current.value+(e.clientX-drag.current.x)*1.3)}} onPointerUp={()=>drag.current=null}>{ch===" "?" ":ch}{selected===i&&!blue&&!fixed&&caretVisible&&<span className="kerning-caret" style={{transform:`translateX(${values[i]/2000}em)`}} />}{!blue&&caretVisible&&atStart&&<span className="kerning-caret" style={{left:0,right:"auto"}} />}{!blue&&caretVisible&&atEnd&&<span className="kerning-caret" />}</span>})}</div> }

function Result({drill,values,targets,accuracy,comparison,setComparison,comparisonView,setComparisonView,next,saveCurrentAnswerAsTarget,restoreTeachingTarget,hasCustomTarget}:{drill:Drill;values:number[];targets:number[];accuracy:number;comparison:number;setComparison:(v:number)=>void;comparisonView:"blend"|"user"|"target";setComparisonView:(v:"blend"|"user"|"target")=>void;next:()=>void;saveCurrentAnswerAsTarget:()=>void;restoreTeachingTarget:()=>void;hasCustomTarget:boolean}) { return <div className="mt-7"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-center"><div><p className="text-xs font-medium tracking-[.12em] text-[#6e6e73]">ACCURACY</p><p className="mt-1 text-4xl font-medium tracking-[-.05em]">{accuracy.toFixed(1)}<span className="ml-1 text-xl">%</span></p></div><button onClick={next} className="w-fit bg-[#171719] px-4 py-2.5 text-sm text-white">次のドリル →</button></div><div className="mt-7 border-t pt-6 hairline"><div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-start"><div className="w-full max-w-sm"><div className="flex items-center gap-4"><button onClick={()=>setComparisonView("user")} aria-pressed={comparisonView==="user"} className={`text-xs ${comparisonView==="user"?"font-medium text-[#171719]":"text-[#6e6e73]"}`}>あなた</button><input className="track flex-1" type="range" value={comparison} onChange={e=>{setComparison(+e.target.value);setComparisonView("blend");}} /><button onClick={()=>setComparisonView("target")} aria-pressed={comparisonView==="target"} className={`text-xs ${comparisonView==="target"?"font-medium text-[#146ef5]":"text-[#146ef5]"}`}>正解</button></div><p className="mt-3 text-xs text-[#6e6e73]">黒：あなたに青：正解を重ねて比較</p></div><div className="flex flex-col items-start gap-2 lg:items-end"><div className="flex flex-wrap gap-3"><button onClick={saveCurrentAnswerAsTarget} className="border px-4 py-2.5 text-sm hairline">{hasCustomTarget?"この回答を正解として更新":"この回答を正解にする"}</button>{hasCustomTarget&&<button onClick={restoreTeachingTarget} className="border px-4 py-2.5 text-sm hairline">教材の正解に戻す</button>}</div><p className="text-xs text-[#6e6e73]">このブラウザ内だけに保存されます。</p></div></div></div><div className="mt-6 border-t pt-6 hairline"><p className="text-xs font-medium tracking-[.12em] text-[#6e6e73]">解説</p><div className="mt-5 grid gap-x-12 gap-y-6 md:grid-cols-2">{drill.pairs.map((p,i)=>{const pos=p.index ?? drill.text.indexOf(p.left+p.right);const v=values[pos]??0;const target=targets[pos]??p.target;return <article key={i}><div className="flex items-baseline gap-4"><h3 className="text-lg font-medium">{p.left === " "?"Space":p.left}{p.right === " "?" Space":p.right}</h3><span className="font-mono text-xs text-[#6e6e73]">あなた {v>0?"+":""}{v}　正解 {target>0?"+":""}{target}　差分 {v-target>0?"+":""}{v-target}</span></div><p className="mt-2 max-w-md text-sm leading-6 text-[#6e6e73]">{p.note}</p></article>})}</div></div></div>}
