...
export default function KerningDrill() {
  ...
  const init = (next: Drill) => {
    setDrill(next);
    setValues(Array.from({ length: next.text.length - 1 }, (_, index) => isFixedSpace(index, next) ? pairAt(next, index).target : 0));
  ...
  const update = (index:number, value:number) => {
    if (isFixedSpace(index)) return;
    setValues(v=>v.map((x,i)=>i===index?Math.max(-150,Math.min(180,Math.round(value))):x));
  };
  useEffect(() => { const handler=(e:KeyboardEvent)=>{ if(screen!=="drill" || !["ArrowLeft","ArrowRight"].includes(e.key)) return; e.preventDefault(); if(e.altKey) { update(selected, values[selected]+(e.key==="ArrowLeft"?-10:10)); return; } const direction=e.key==="ArrowLeft"?-1:1; setSelected(current => { let next=current+direction; while(next>=0 && next<drill.text.length-1 && isFixedSpace(next)) next+=direction; return Math.max(0,Math.min(drill.text.length-2,next)); }); }; window.addEventListener("keydown",handler); return()=>window.removeEventListener("keydown",handler); });
  ...
  return <main className="min-h-screen px-5 py-5 sm:px-10 sm:py-8">
    ...
    {screen !== "home" && <section className="mx-auto max-w-6xl py-9"><div className="flex items-center justify-between"><div><p className="text-xs font-medium tracking-[.12em] text-[#6e6e73]">{drill.level} · DRILL</p><h2 className="mt-1 text-xl font-medium tracking-[-.03em]">余白を見ながら、文字を選択</h2></div><label className="flex items-center gap-3 text-xs text-[#6e6e73]">Font<select value={font} onChange={e=>setFont(e.target.value)} className="border bg-white px-2 py-1.5 text-[#171719] hairline">{fonts.map(f=><option key={f}>{f}</option>)}</select></label></div>
      <div className="mt-8 min-h-[390px] border-y py-16 hairline"><div className="overflow-x-auto px-2 text-center"><div className="relative inline-block text-left"><KerningText text={drill.text} values={renderedValues} font={font} selected={selected} onSelect={setSelected} drag={drag} update={update} isFixedSpace={isFixedSpace} faded={screen==="result" && comparison>0} />{screen==="result" && <div className="pointer-events-none absolute left-0 top-0 opacity-85"><KerningText text={drill.text} values={targets} font={font} selected={-1} onSelect={()=>{}} drag={drag} update={()=>{}} isFixedSpace={isFixedSpace} blue /></div>}</div></div></div>
      {screen === "drill" ? <div className="mt-7 flex flex-col justify-between gap-7 sm:flex-row sm:items-end"><div><div className="flex items-center gap-3 text-sm"><span className="font-medium">{`${drill.text[selected]}${drill.text[selected+1]}`}</span><span className="font-mono text-[#6e6e73]">{values[selected] > 0 ? "+" : ""}{values[selected]}</span><span className="text-xs text-[#6e6e73]">/1000 em</span></div><div className="mt-3 flex items-center gap-4"><input className="track" type="range" min="-150" max="180" value={values[selected]} onChange={e=>update(selected,+e.target.value)} /><span className="text-xs text-[#6e6e73]">または <span className="key">⌥</span> + <span className="key">←</span><span className="key">→</span><span className="ml-2">単語間スペースは固定</span></span></div></div><button onClick={submit} className="bg-[#171719] px-5 py-3 text-sm font-medium text-white">採点する <span className="ml-5 text-[#b4b4b8]">→</span></button></div> : <Result drill={drill} values={values} targets={targets} accuracy={accuracy} comparison={comparison} setComparison={setComparison} showAnswer={showAnswer} setShowAnswer={setShowAnswer} next={next} saveCurrentAnswerAsTarget={saveCurrentAnswerAsTarget} hasCustomTarget={Boolean(customTargets[drill.text])} />}</section>}
  </main>;
}

function KerningText({text,values,font,selected,onSelect,drag,update,isFixedSpace,blue=false,faded=false}:{text:string;values:number[];font:string;selected:number;onSelect:(n:number)=>void;drag:React.MutableRefObject<{x:number;value:number}|null>;update:(n:number,v:number)=>void;isFixedSpace:(index:number)=>boolean;blue?:boolean;faded?:boolean}) { return <div aria-label="Kerning canvas" style={{fontFamily:font,color:blue?"#146ef5":"#171719",opacity:faded?.22:1}} className="inline-block whitespace-nowrap text-[clamp(56px,11vw,130px)] font-medium leading-none tracking-[-.055em]">{[...text].map((ch,i)=>{const fixed=i>=text.length-1 || isFixedSpace(i);return <span key={i} className={`char ${selected===i&&!blue&&!fixed?"active":""}`} style={{marginRight:i<text.length-1?`${values[i]/1000}em`:0, transition:"margin-right .4s cubic-bezier(.2,.8,.2,1)"}} onClick={()=>!blue&&!fixed&&onSelect(i)} onPointerDown={e=>{if(blue||fixed)return; onSelect(i); drag.current={x:e.clientX,value:values[i]}; e.currentTarget.setPointerCapture(e.pointerId)}} onPointerMove={e=>{if(!drag.current||blue||fixed)return; update(i,drag.current.value+(e.clientX-drag.current.x)*1.3)}} onPointerUp={()=>drag.current=null}>{ch===" "?" ":ch}{selected===i&&!blue&&!fixed&&<span className="kerning-caret" style={{transform:`translateX(${values[i]/2000}em)`}} />}</span>})}</div> }

function Result({drill,values,targets,accuracy,comparison,setComparison,showAnswer,setShowAnswer,next,saveCurrentAnswerAsTarget,hasCustomTarget}:{drill:Drill;values:number[];targets:number[];accuracy:number;comparison:number;setComparison:(v:number)=>void;showAnswer:boolean;setShowAnswer:(v:boolean)=>void;next:()=>void;saveCurrentAnswerAsTarget:()=>void;hasCustomTarget:boolean}) { return <div className="mt-7"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-center"><div><p className="text-xs font-medium tracking-[.12em] text-[#6e6e73]">ACCURACY</p><p className="mt-1 text-4xl font-medium tracking-[-.05em]">{accuracy.toFixed(1)}<span className="ml-1 text-xl">%</span></p></div><div className="flex flex-wrap gap-3"><button onClick={saveCurrentAnswerAsTarget} className="border px-4 py-2.5 text-sm hairline">{hasCustomTarget?"この回答を正解として更新":"この回答を正解にする"}</button><button onClick={()=>setShowAnswer(!showAnswer)} className="border px-4 py-2.5 text-sm hairline">{showAnswer?"解説を閉じる":"答えと比較"}</button><button onClick={next} className="bg-[#171719] px-4 py-2.5 text-sm text-white">次のドリル →</button></div></div><p className="mt-3 text-xs text-[#6e6e73]">保存した正解は、このブラウザで次回以降の同じ問題に使われます。</p><div className="mt-7 border-t pt-6 hairline"><div className="flex max-w-sm items-center gap-4"><span className="text-xs">あなた</span><input className="track flex-1" type="range" value={comparison} onChange={e=>setComparison(+e.target.value)} /><span className="text-xs text-[#146ef5]">正解</span></div><p className="mt-3 text-xs text-[#6e6e73]">左端とベースラインを固定し、黒：あなたを青：正解へ補間</p></div>{showAnswer&&<div className="mt-8 grid gap-x-12 gap-y-6 border-t pt-7 hairline md:grid-cols-2">{drill.pairs.map((p,i)=>{const pos=p.index ?? drill.text.indexOf(p.left+p.right);const v=values[pos]??0;const target=targets[pos]??p.target;return <article key={i}><div className="flex items-baseline gap-4"><h3 className="text-lg font-medium">{p.left === " "?"Space":p.left}{p.right === " "?" Space":p.right}</h3><span className="font-mono text-xs text-[#6e6e73]">あなた {v>0?"+":""}{v}　正解 {target>0?"+":""}{target}　差分 {v-target>0?"+":""}{v-target}</span></div><p className="mt-2 max-w-md text-sm leading-6 text-[#6e6e73]">{p.note}</p></article>})}</div>}</div>}
