...
export default function KerningDrill() {
  ...
  return <main className="min-h-screen px-5 py-5 sm:px-10 sm:py-8">
    ...
    {screen !== "home" && <section className="mx-auto max-w-6xl py-9"><div className="flex items-center justify-between"><div><p className="text-xs font-medium tracking-[.12em] text-[#6e6e73]">{isReview ? "復習" : drill.level} · DRILL　{guidedIndex + 1}/{guidedQueue.length || 12}</p><h2 className="mt-1 text-xl font-medium tracking-[-.03em]">余白を見ながら、文字を選択</h2></div><p className="text-xs text-[#6e6e73]">{editorMode && <span className="mr-3 text-[#146ef5]">編集モード</span>}フォント　<span className="text-[#171719]">{font}</span></p></div>
      <div className={`mt-8 border-y hairline ${screen === "result" ? "min-h-[300px] py-10" : "min-h-[390px] py-16"}`}><div className="overflow-x-auto px-2 text-center"><div className="relative inline-block text-left"><KerningText text={drill.text} values={renderedValues} font={font} selected={selected} onSelect={activatePair} drag={drag} update={update} isFixedSpace={isFixedSpace} caretVisible={screen === "drill" && caretVisible} opacity={screen === "result" ? comparisonView === "user" ? 1 : comparisonView === "target" ? 0 : 0.05 + 0.95 * (1 - comparison / 100) : 1} />{(screen==="result" || showMetrics) && <div className="pointer-events-none absolute left-0 top-0" style={{opacity: screen === "result" ? comparisonView === "user" ? 0 : comparisonView === "target" ? 1 : 0.05 + 0.95 * (comparison / 100) : 0.85}}><KerningText text={drill.text} values={targets} font={font} selected={-1} onSelect={()=>{}} drag={drag} update={()=>{}} isFixedSpace={isFixedSpace} caretVisible={false} blue /></div>}</div></div></div>
